...
Optional: `SYNO_PHOTOS_URL`, `SYNO_PHOTOS_USER`, `SYNO_PHOTOS_PASSWORD` — enable the Synology Photos "veto" filter (`src/lib/photo-vetoes.ts`). Synology Photos stores manual tags in its own database, never in the image files' EXIF/XMP, so the filter queries the Photos Web API (as the library owner's account — personal-space tags are only visible to their owner) for items tagged `veto` and skips them in `/api/photos/random`. Missing vars or an unreachable NAS disable the filter cleanly (fail-open, one console warning); the veto set is cached for 6h. The embedded-keyword veto check in `photo-meta.ts` also remains for files with real XMP/IPTC keywords.
